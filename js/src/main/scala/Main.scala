...
object Main {
  ...
  def update(str: String): Unit = {
    ...
    def report(diag: Diagnostic): Str = {
      ...
      val sctx = Message.mkCtx(diag.allMsgs.iterator.map(_._1), true, "?")
    ...
    val tryRes = try {
      ...
      val parser = new NewParser(origin, tokens, true, throw _, dbg = false, N) {
      ...
      parser.parseAll(parser.typingUnit) match {
        case tu =>
          ...
          val expStr = exp.showIn(ShowCtx.mk(exp :: Nil, true), 0).stripSuffix("\n")
            .replaceAll("  ", "&nbsp;&nbsp;")
            .replaceAll("\n", "<br/>")
